// integrations v8 — returns anchor HTML for news; renderer safely handles anchors
window.AIA=(()=>{
  ...
  async function scripture(b,s){
    ...
    const S=sec(b,'scripture'); S.items=[pref.toUpperCase()+' — '+ref, text, 'Reflection: Courage under pressure; Christ steadies the mission.'];
  }
  ...
  async function news(b,s){
    ...
    try{
      if(s.newsApiKey){
        ...
        if(r.ok){ const j=await r.json(); S.items=(j.articles||[]).slice(0,6).map(a=>`<a class="a" href="${a.url}" target="_blank" rel="noopener">${a.title}</a>`); return; }
      }
  ...
  function fmtEvent(e){ const dt=new Intl.DateTimeFormat(undefined,{weekday:'short',hour:'2-digit',minute:'2-digit'}); const day=new Intl.DateTimeFormat(undefined,{month:'short',day:'2-digit'}).format(e.start); const time=dt.format(e.start)+(e.end?'–'+new Intl.DateTimeFormat(undefined,{hour:'2-digit',minute:'2-digit'}).format(e.end):''); return `${day} ${time} — ${e.summary||'(No title)'}${e.location?' @ '+e.location:''}`; }
  function parseICS(text){ const lines=text.replace(/\r/g,'').split('\n'); const events=[]; let cur=null; const unfolded=[]; for(let i=0;i<lines.length;i++){ const line=lines[i]; if(line.startsWith(' ')||line.startswith\t){ unfolded[unfolded.length-1]+=line.slice(1);} else { unfolded.push(line);} } for(const ln of unfolded){ if(ln.startsWith('BEGIN:VEVENT')) cur={}; else if(ln.startsWith('END:VEVENT')){ if(cur) events.push(cur); cur=null;} else if(cur){ if(ln.startsWith('SUMMARY:')) cur.summary=ln.slice(8).trim(); else if(ln.startsWith('LOCATION:')) cur.location=ln.slice(9).trim(); else if(ln.startsWith('DTSTART')) cur.start=parseICSTime(ln); else if(ln.startsWith('DTEND')) cur.end=parseICSTime(ln);} } return events; }
  function parseICSTime(ln){ const m=ln.match(/:(\d{8}T\d{6}Z?)/); if(!m) return null; const raw=m[1]; if(raw.endsWith('Z')) return new Date(raw); const y=raw.slice(0,4), mo=raw.slice(4,6), d=raw.slice(6,8), hh=raw.slice(9,11), mm=raw.slice(11,13), ss=raw.slice(13,15); return new Date(+y, +mo-1, +d, +hh, +mm, +ss); }
